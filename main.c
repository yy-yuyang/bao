...
static PlannedStmt* bao_planner(Query *parse, const char *query_string,
  ...
  elog(WARNING, "plan_query ON!!!");
  plan = bao_plan_query(parse, query_string, cursorOptions, boundParams);
  elog(WARNING, "plan_query!!!!");
...
static void bao_ExplainOneQuery(Query* query, int cursorOptions, IntoClause* into,
  ...
  // plan = bao_planner(query, queryString, cursorOptions, params);
  elog(WARNING, "bao_planner...%d", plan->type);
  ...
  buffer_json = bao_buffer_state();
  elog(WARNING, "ExplainOnePlan%d", plan->type);
  plan_json = bao_plan_to_json(plan);
  // elog(WARNING, "Finish%s", plan_json);
  ...
  elog(WARNING, "OK>>>3");
  bao_plan = bao_plan_query(query, queryString, cursorOptions, params);
  elog(WARNING, "OK>>>4");
  ...
  ExplainOnePlan(plan, into, es, queryString,
                 params, queryEnv, &plan_duration, NULL);
}
