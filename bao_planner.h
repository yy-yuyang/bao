...
BaoPlan *bao_plan_query(Query *parse, const char *query_string, int cursorOptions, ParamListInfo boundParams);
...
static PlannedStmt* bao_plan_arm(int arm, Query* parse, const char *query_string,
                             int cursorOptions, ParamListInfo boundParams) {

  PlannedStmt* plan = NULL;
  Query* query_copy = copyObject(parse); // create a copy of the query plan
  elog(WARNING, "bao_plan_arm%d", arm);
  if (arm == -1) {
...
static char* bao_plan_to_json(PlannedStmt* plan) {
  char* buf;
  size_t json_size;
  FILE* stream;
  ...
  // elog(WARNING, "plan_to_json:%d", transformed_plan->node_type);
...
BaoPlan* bao_plan_query(Query *parse, const char *query_string, int cursorOptions, ParamListInfo boundParams) {
  ...
  elog(WARNING, "WHY??");
  plan->query_info->buffer_json = bao_buffer_state();
  elog(WARNING, "enable_bao_selection???%d", enable_bao_selection);
  ...
  if (!enable_bao_selection) {
    ...
    // enable_nestloop.
    elog(WARNING, "WHERE???0");
    plan->plan = bao_plan_arm(-1, parse, query_string, cursorOptions, boundParams);
    
    plan->query_info->plan_json = bao_plan_to_json(plan->plan);
    elog(WARNING, "enable_bao_selection");
    return plan;
  }
  elog(WARNING, "WHERE???1");
  conn_fd = connect_to_bao(bao_host, bao_port);
  if (conn_fd == -1) {
    elog(WARNING, "Unable to connect to Bao server.");
    return NULL;
  }
  elog(WARNING, "WHERE2.5");
  memset(plan_for_arm, 0, BAO_MAX_ARMS*sizeof(PlannedStmt*));
  elog(WARNING, "WHERE???2");
  write_all_to_socket(conn_fd, START_QUERY_MESSAGE);
  for (int i = 0; i < bao_num_arms; i++) {
    // Plan the query for this arm.
    elog(WARNING, "WHERE???3");
    query_copy = copyObject(parse);
    elog(WARNING, "WHERE???4");
    plan_for_arm[i] = bao_plan_arm(i, query_copy, query_string, cursorOptions, boundParams);
    elog(WARNING, "WHERE???5");
    // Transform it into JSON, transmit it to the Bao server.
    json_for_arm[i] = bao_plan_to_json(plan_for_arm[i]);
